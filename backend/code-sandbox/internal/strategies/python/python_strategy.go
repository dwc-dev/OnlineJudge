...
timeout "$timeout_sec"s /usr/bin/time -v python /sandbox/main.py < "$input_file"
...
func NewPythonStrategy(runConfig *types.RunConfig, dockerClient *docker.DockerClient) *PythonStrategy {
...
func (s *PythonStrategy) Prepare(ctx context.Context) error {
	var err error
	s.containerID, err = s.dockerClient.CreateContainer(ctx, "oj-python", s.memoryLimitMiB+128, s.stackLimitMiB, 1)
	if err != nil {
		return err
	}
	...
	err = s.dockerClient.StringToContainerFile(ctx, s.containerID, "/sandbox", "main.py", s.code)
	if err != nil {
		return err
	}
	err = s.dockerClient.StringToContainerFile(ctx, s.containerID, "/sandbox", "python_runner.sh", pythonRunner)
	if err != nil {
		return err
	}
	_, err = s.dockerClient.ExecInContainer(ctx, s.containerID, []string{"chmod", "+x", "/sandbox/python_runner.sh"})
	if err != nil {
		return err
	}
	return nil
...
func (s *PythonStrategy) Compile(ctx context.Context) (*types.Result, error) {
	// python无需编译
	return nil, nil
}

func (s *PythonStrategy) Execute(ctx context.Context) (*types.Result, error) {
	...
	for idx := range len(s.inputList) {
		...
		result, err := s.dockerClient.ExecInContainer(ctx, s.containerID,
			[]string{"sh", "-c",
				"/sandbox/python_runner.sh " + inputPath + " " + fmt.Sprintf("%.3f", timeout)})
		if err != nil {
			return nil, err
		}
		isTimeout := false
		...
		if result.ExitCode == 124 {
			...
			strings.Contains(result.ErrorOutput, "signal 11") ||
			strings.Contains(result.ErrorOutput, "RecursionError") {
			// 栈溢出
			result.StandardOutput = ""
