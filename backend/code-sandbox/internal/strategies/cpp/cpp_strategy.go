...
func NewCppStrategy(runConfig *types.RunConfig, dockerClient *docker.DockerClient) *CppStrategy {
	return &CppStrategy{
		code:               runConfig.Code,
...
func (s *CppStrategy) Prepare(ctx context.Context) error {
	var err error
	s.containerID, err = s.dockerClient.CreateContainer(ctx, "oj-cpp", s.memoryLimitMiB+128, s.stackLimitMiB, 1)
	if err != nil {
		return err
	}
	...
	err = s.dockerClient.StringToContainerFile(ctx, s.containerID, "/sandbox", "main.cpp", s.code)
	if err != nil {
		return err
	}
	err = s.dockerClient.StringToContainerFile(ctx, s.containerID, "/sandbox", "cpp_runner.sh", cppRunner)
	if err != nil {
		return err
	}
	_, err = s.dockerClient.ExecInContainer(ctx, s.containerID, []string{"chmod", "+x", "/sandbox/cpp_runner.sh"})
	if err != nil {
		return err
	}
	return nil
...
func (s *CppStrategy) Compile(ctx context.Context) (*types.Result, error) {
	result, err := s.dockerClient.ExecInContainer(ctx, s.containerID,
		[]string{"g++", "-o", "/sandbox/main", "/sandbox/main.cpp"})
	if err != nil {
		return nil, err
	}
...
func (s *CppStrategy) Execute(ctx context.Context) (*types.Result, error) {
	...
	for idx := range len(s.inputList) {
		...
		result, err := s.dockerClient.ExecInContainer(ctx, s.containerID,
			[]string{"sh", "-c",
				"/sandbox/cpp_runner.sh " + inputPath + " " + fmt.Sprintf("%.3f", timeout)})
		if err != nil {
			return nil, err
		}
		isTimeout := false
