...
func NewGolangStrategy(runConfig *types.RunConfig, dockerClient *docker.DockerClient) *GolangStrategy {
...
func (s *GolangStrategy) Prepare(ctx context.Context) error {
	var err error
	s.containerID, err = s.dockerClient.CreateContainer(ctx, "oj-golang", s.memoryLimitMiB+128, s.stackLimitMiB, 1)
	if err != nil {
		return err
	}
	...
	err = s.dockerClient.StringToContainerFile(ctx, s.containerID, "/sandbox", "main.go", s.code)
	if err != nil {
		return err
	}
	err = s.dockerClient.StringToContainerFile(ctx, s.containerID, "/sandbox", "golang_runner.sh", golangRunner)
	if err != nil {
		return err
	}
	_, err = s.dockerClient.ExecInContainer(ctx, s.containerID, []string{"chmod", "+x", "/sandbox/golang_runner.sh"})
	if err != nil {
		return err
	}
	return nil
...
func (s *GolangStrategy) Compile(ctx context.Context) (*types.Result, error) {
...
func (s *GolangStrategy) Execute(ctx context.Context) (*types.Result, error) {
	...
	for idx := range len(s.inputList) {
		...
		result, err := s.dockerClient.ExecInContainer(ctx, s.containerID,
			[]string{"sh", "-c",
				"/sandbox/golang_runner.sh " + inputPath + " " + fmt.Sprintf("%.3f", timeout)})
		if err != nil {
			return nil, err
		}
		isTimeout := false
